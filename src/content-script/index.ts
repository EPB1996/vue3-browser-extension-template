...
chrome.storage.sync.get(['devoteam-extension-settings'], (result) => {
    const contentScriptVisible =
        result['devoteam-extension-settings']['content_script_settings'][
            'options'
...
chrome.storage.onChanged.addListener((changes) => {
    const contentScriptVisible =
        changes['devoteam-extension-settings'].newValue[
...
function setPosition(x: any, y: any) {
    ...
    const crxIframe = contentScript.querySelector(
        '.crx-iframe'
    ) as HTMLDivElement
    ...
    const menuSize = getComputedStyle(
        document.documentElement
    ).getPropertyValue('--menu-size')
...
function createContentScript() {
 ...
 `,
  ...
  iframe.onmouseleave = () => {
    ...
    const menu = new DOMParser().parseFromString(`<div ></div>`, 'text/html')
        .body.firstElementChild as HTMLDivElement
